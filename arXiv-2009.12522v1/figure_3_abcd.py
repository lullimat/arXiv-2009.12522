...
for _psi in psis:
    ...
    for _stencil in [E6_P2F6_sym, E6_P4F6_sym]:
        _data_swap = ManageData(dump_file = flat_files_E6[StencilPsiKey(_stencil, _psi)])
        _is_file_there = _data_swap.Read()
        if not _is_file_there:
            raise Exception("Could not find file!", 
                            flat_files_E6[StencilPsiKey(_stencil, _psi)])
...
for _psi in psis:
    for _stencil in [E6_P2F6_sym, E6_P4F6_sym]:
        ...
        for G in Gs[_psi]:
            ...
            delta_p['G=' + str(G)]['B2F6']['P4Iso=' + E_sym_YN[_stencil]]['droplet'] = \
...
black_labels = []
G = -2.6
red_p, = ax1.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 
                  delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 'x', color = 'red', 
                  markersize = mark_s, label = r'$\boldsymbol{E}^{(6)}_{P4,F6}$')

blue_p, = ax1.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], 
                   delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], '+', color = 'blue', 
                   markersize = mark_s, label = r'$\boldsymbol{E}^{(6)}_{P2,F6}$')

line_swap, = ax1.plot(rm1_axis, 
                      rm1_axis * sigma_f['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], 
                      label = '$Gc_s^2=' + str(G) + '$', color = 'black')

black_lines.append(line_swap)

for G in [-3.1, -3.6]:
    ax1.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 
             delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 'x', color = 'red', 
             markersize = mark_s)

    ax1.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], 
             delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], '+', color = 'blue', 
             markersize = mark_s)
...
G = -1.4
ax2.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 
         delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'], 'x', color = 'red',
         markersize = mark_s)

ax2.plot(1./gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], 
         delta_p['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'], '+', color = 'blue',
         markersize = mark_s)
...
ax2.text(2.5e-3, 1.1e-3, '$\\psi = 1 - \\exp(-n)$', fontsize=11)
ax2.text(0.038, 1.36e-3, '$(b)$', fontsize=f_s)
...
ax3.text(-0.5, -8e-12, '$\\psi = \\exp(-1/n)$', fontsize=11)
ax3.text(39, 1.7e-12, '$(c)$', fontsize=f_s)

### INSET
ax3i = inset_axes(ax3, width='35%', height='70%', loc = 'lower right')
...
ax4.text(-0.5, -3e-11, '$\\psi = 1 - \\exp(-n)$', fontsize=11)
ax4.text(39, 3.5e-12, '$(d)$', fontsize=f_s)

### INSET
ax4i = inset_axes(ax4, width='35%', height='70%', loc = 'lower right')
...
plt.savefig(reproduced_figures / 'figure_3_E6.png',
