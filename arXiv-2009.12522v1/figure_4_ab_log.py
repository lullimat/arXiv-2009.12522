...
for _psi in [psis[0]]:
    for _stencil in [E6_P2F6_sym, E6_P4F6_sym]:
        _data_swap = ManageData(dump_file = laplace_files_E6[StencilPsiKey(_stencil, _psi)])
...
norm_2iso_min, norm_2iso_max = np.log(np.amin(norm_2iso)), np.log(np.amax(norm_2iso))
norm_2niso_min, norm_2niso_max = np.log(np.amin(norm_2niso)), np.log(np.amax(norm_2niso))

norm_2iso_lin = ((np.log(norm_2iso) - norm_2iso_min)/((norm_2iso_max - norm_2iso_min)/n_q_lin))
norm_2niso_lin = ((np.log(norm_2niso) - norm_2niso_min)/((norm_2niso_max - norm_2niso_min)/n_q_lin))

# need to manually select the radius
# print(gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'])
sim_2iso_rad = gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'Yes']['droplet'][-4]
sim_2niso_rad = gibbs_rad['G=' + str(G)]['B2F6']['P4Iso=' + 'No']['droplet'][-4]
...
ax1.text(LX//2 + 30, LX//2 + 82, r'$\boldsymbol{E}^{(6)}_{P4,F6}$', color = 'red', fontsize = f_s)
...
im1 = ax1.imshow(norm_2iso_lin, cmap = 'rainbow', vmin = 0., interpolation='hermite')
ax1.plot([LX//2 - 0.5], [LX//2 - 0.5], 'o', color = 'black', markersize = 2)
ax1.text(LX - 55, LX//2 + 83, '$(A)$', fontsize = f_s)
cb1 = fig.colorbar(im1, ticks = np.linspace(0, 12, 4))
cb1.set_label('$\\lfloor 12\; \\log(u/u_{m})/\\log(u_M/u_m)\\rfloor$', labelpad=+18, rotation=-90, fontsize = f_s)

#################### PANEL (b) ####################
ax2 = plt.subplot2grid((2,1), (1,0), colspan=1, rowspan=1)

ax2.text(LX//2 + 30, LX//2 + 82, r'$\boldsymbol{E}^{(6)}_{P2,F6}$', color = 'blue', fontsize = f_s)
...
im2 = ax2.imshow(norm_2niso_lin, cmap = 'rainbow', vmin = 0., interpolation='hermite')
ax2.plot([LX//2 + 0.5], [LX//2 + 0.5], 'o', color = 'black', markersize = 2)
ax2.text(LX - 55, LX//2 + 83, '$(B)$', fontsize = f_s)
cb2 = fig.colorbar(im2, ticks = np.linspace(0, 12, 4))
cb2.set_label('$\\lfloor 12\; \\log(u/u_{m})/\\log(u_M/u_m)\\rfloor$', labelpad=+18, rotation=-90, fontsize = f_s)
...
plt.savefig(reproduced_figures / 'figure_4_ab_log.png',
