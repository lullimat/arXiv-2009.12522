...
E_sym_YN = {E6_P2F6_sym: 'No', E6_P4F6_sym: 'Yes', E10_P2F10_sym: 'No', E10_P4F6_sym: 'Yes',
...
for _psi in [psis[0]]:
    for _stencil in [E12_P2F12_sym, E12_P4F6_sym]:
        _data_swap = ManageData(dump_file = laplace_files_E12[StencilPsiKey(_stencil, _psi)])
        _is_file_there = _data_swap.Read()
        if not _is_file_there:
            raise Exception("Could not find file!", 
                            laplace_files_E12[StencilPsiKey(_stencil, _psi)])        
...
ax1.text(_e_label_pos[0], _e_label_pos[1], r'$\boldsymbol{E}^{(12)}_{P4,F6}$',
         transform = ax1.transAxes, color = 'red', fontsize = f_s)

ax1.text(_panel_label_pos[0], _panel_label_pos[1], '$(g)$',
...
ax2.text(_e_label_pos[0], _e_label_pos[1], r'$\boldsymbol{E}^{(12)}_{P2,F12}$',
         transform = ax2.transAxes, color = 'blue', fontsize = f_s)
ax2.text(_panel_label_pos[0], _panel_label_pos[1], '$(h)$',
...
plt.savefig(reproduced_figures / 'figure_5_gh.png',
